...
void benchmark(const bpo::variables_map& opts) {
    ...
    for (size_t r = 0; r < repeat; ++r) {
        ...
        std::fprintf(stderr, "TS pid=%zu wall=%lld pre_sync\n", pid, (long long)wall_ms());
        network->sync();
        std::fprintf(stderr, "TS pid=%zu wall=%lld post_sync\n", pid, (long long)wall_ms());
        StatsPoint start(*network);
        auto _t_off_start = std::chrono::high_resolution_clock::now();
        auto preproc = off_eval.run(input_pid_map);
        auto _t_off_end = std::chrono::high_resolution_clock::now();
        double _off_ms = std::chrono::duration<double, std::milli>(_t_off_end - _t_off_start).count();
        std::fprintf(stderr, "TIMER pid=%zu off_eval.run()=%.2f ms\n", pid, _off_ms);
        std::fprintf(stderr, "TS pid=%zu wall=%lld post_offline\n", pid, (long long)wall_ms());
        ...
        std::fprintf(stderr, "TS pid=%zu wall=%lld post_setrandom\n", pid, (long long)wall_ms());

        // network->sync();

        // StatsPoint start(*network);
        auto _t_on_start = std::chrono::high_resolution_clock::now();
        for (size_t i = 0; i < circ.gates_by_level.size(); ++i) {
            eval.evaluateGatesAtDepth(i);
            if (i == 0 || i == 1 || i == 5 || i == 25 || i == 50 || i == 75 || i == 99) {
                std::fprintf(stderr, "TS pid=%zu wall=%lld online_layer=%zu\n",
                             pid, (long long)wall_ms(), i);
            }
        }
        auto _t_on_end = std::chrono::high_resolution_clock::now();
        double _on_ms = std::chrono::duration<double, std::milli>(_t_on_end - _t_on_start).count();
        std::fprintf(stderr, "TIMER pid=%zu online_loop=%.2f ms\n", pid, _on_ms);
