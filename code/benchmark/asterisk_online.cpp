...
void benchmark(const bpo::variables_map& opts) {
    ...
    for (size_t r = 0; r < repeat; ++r) {
        ...
        // Drain delay: under taprio gating the dealer (P0) finishes the
        // online phase in ms while regular parties are still receiving
        // its broadcasts. Pump-thread NACKs can't catch up if P0 exits
        // immediately. Sleep on the dealer side so the pump keeps
        // serving retransmits until the others drain.
        if (pid == 0) std::this_thread::sleep_for(std::chrono::seconds(3));
